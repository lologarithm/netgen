...
func Write(ctx ngen.Context, msg ngen.Message, buffer *ngen.Buffer) error {
	switch msg.MsgType() {
		case ngen.MessageTypeContext:
			...
			buffer.Buf = msg.(ngen.Context).Serialize()
			return nil
%s
		default:
...
`
	writeCase := `		case %sMsgType:
				// TODO: Handle both pointers and structs
				%sSerialize(ctx, msg.(%s), content)
				return content.Err`
	caseBuffer.Reset()
	for _, t := range messages {
		caseBuffer.WriteString(fmt.Sprintf(writeCase, t.Name, t.Name, goName(t)))
	}
...
func GoSerializers(msg Message, messages []Message, messageMap map[string]Message, enums []Enum, enumMap map[string]Enum) string {
	gobuf := &bytes.Buffer{}
	gobuf.WriteString(fmt.Sprintf("\n\nfunc %sSerialize(ctx *ngen.Context, msg %s, buffer *ngen.Buffer) {\n", msg.Name, goName(msg)))
	if msg.Versioned {
...
func WriteGoSerializeField(f MessageField, scopeDepth int, buf *bytes.Buffer, messages map[string]Message, enums map[string]Enum) {
	...
	default:
		if _, ok := messages[f.Type]; ok || f.Interface {
			...
			buf.WriteString(n)
			buf.WriteString(".Serialize(buffer[idx:], settings)\n")
			writeTabScope(buf, scopeDepth)
			buf.WriteString("idx += ")
			buf.WriteString(n)
			buf.WriteString(".Len()\n")
	...
	default:
		...
		if _, ok := messages[f.Type]; ok {
			...
			if f.Pointer {
				...
				buf.WriteString(" = ")
				buf.WriteString(f.Type)
				buf.WriteString("Deserialize(buffer, settings)\n")
				writeTabScope(buf, scopeDepth)
				buf.WriteString("\t")
				buf.WriteString(n)
				buf.WriteString(" = &")
